...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const menuItems = [
    ...
    { icon: Image, label: "Banners", href: "/admin-la-educacao/banners" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-purple-50/30 to-pink-50/20">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 h-full w-72 bg-gradient-to-b from-[#9d197d] via-purple-900 to-purple-950 text-white shadow-2xl flex flex-col">
        {/* Header da Sidebar */}
        ...
        <div className="p-4 border-t border-white/10 bg-black/10 backdrop-blur-sm">
          <div className="mb-3 px-3 py-2 bg-white/5 rounded-lg border border-white/10">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center font-bold text-white text-sm shadow-lg">
                {adminUser.name?.charAt(0) || "A"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate">{adminUser.name || "Administrador"}</p>
                <p className="text-xs text-purple-300 truncate">{adminUser.email || "admin@laeducacao.com.br"}</p>
              </div>
            </div>
          </div>
          <Button
            variant="ghost"
            className="w-full justify-start text-purple-200 hover:bg-white/10 hover:text-white transition-all duration-200 rounded-lg"
